            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or view_img:  # Add bbox to image
                        label = names[int(cls)]
                        new_img = im0[int(xyxy[1]):int(xyxy[3]), int(xyxy[0]): int(xyxy[2])]
                        new_path = os.path.join('images', '%s.png' % shortuuid.uuid())
                        cv2.imwrite(new_path, new_img)
                        i = i + 1
                        coordinate = [int(xyxy[0]), int(xyxy[1]), int(xyxy[2]), int(xyxy[3]), int(xyxy[0]), int(xyxy[1]), int(xyxy[2]), int(xyxy[3])]
                        json = {"invoice_type": label, "invoice_type_name": converter[label], "file_path": new_path, "coordinate": coordinate}
