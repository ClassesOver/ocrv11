...
import config
from util.tool import get_amount, get_chinese_amount, get_date, get_num, get_page, get_qrcode_data_v2 as get_qrcode_data, save_image
    ...
    if results and len(results) > 0:
        ...
        if boxes is not None and len(boxes) > 0:
            ...
            if labels:
                ...
                for label in SKIP_OCR_LABELS:
                    if label in labels:
                        ...
                        if conf >= CONFIDENCE_THRESHOLD:
                            # 对于 handled_by, verified_by, accountant 标签，保存图片文件并计算checksum
                            if label in ('handled_by', 'verified_by', 'accountant'):
                ...
                if 'col1' in labels and 'col2' in labels:
                    col1_img = labels['col1']
                    col2_img = labels['col2']
                    col1_cells = context.ocr_columns_cells(col1_img, saveImage=saveImage, col_name='col1')
                    col2_cells = context.ocr_columns_cells(col2_img, saveImage=saveImage, col_name='col2')
                    
                    # 根据 col2_cells 的长度对齐，如果 col1_cells 较短则用空字符串填充
                    col2_len = len(col2_cells) if col2_cells else 0
                    col1_len = len(col1_cells) if col1_cells else 0
                    
                    # 确保 col1_cells 长度与 col2_cells 一致
                    if col1_len < col2_len:
                        # col1_cells 较短，用空字符串填充
                        col1_cells = list(col1_cells) + [''] * (col2_len - col1_len)
                    elif col1_len > col2_len:
                        # col1_cells 较长，截断到 col2_cells 的长度
                        col1_cells = col1_cells[:col2_len]
                    col1_cells = ['金额'] + col1_cells
                    col2_cells = ['发票号码']+  col2_cells 
                    # 根据 col2_cells 对齐进行 zip
                    rows = list(zip(col1_cells, col2_cells))
                    stock['line'] = rows
                    stock['line_conf'] = label_confidences.get('line', 0.0) or label_confidences.get('col1', 0.0) or label_confidences.get('col2', 0.0)
                    detected = True
                # 处理 line 标签：进行表格识别
                elif 'line' in labels and force_ocr_table:
                    try:
